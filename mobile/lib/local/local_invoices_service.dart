...
class LocalInvoicesService implements InvoicesService {
      ...
      try {
        await _database.into(_database.invoices).insert(
              InvoicesCompanion.insert(
                ...
                customerWhatsappNumber: Value(prepared.customer.whatsappNumber),
  ...
  Future<InvoiceDetail> _buildInvoiceDetail(String invoiceId) async {
    ...
    return InvoiceDetail(
      ...
      customerWhatsappNumber: invoice.customerWhatsappNumber,
      customerGstin: invoice.customerGstin,
      invoiceDate: invoice.invoiceDate,
      invoiceDatetime: invoice.invoiceDatetime,
      subtotal: double.parse(invoice.subtotal),
      discountTotal: double.parse(invoice.discountTotal),
      taxableTotal: double.parse(invoice.taxableTotal),
      gstTotal: double.parse(invoice.gstTotal),
      ...
      companyBankBranch: invoice.companyBankBranch,
      notes: invoice.notes,
      ...
      items: items
          .map(
            (item) => InvoiceDetailItem(
              ...
              unitPriceExclTax: double.parse(item.unitPriceExclTax),
              unitPriceInclTax: double.parse(item.unitPriceInclTax),
              gstRate: double.parse(item.gstRate),
              cgstRate: double.parse(item.cgstRate),
              sgstRate: double.parse(item.sgstRate),
              igstRate: double.parse(item.igstRate),
              gstAmount: double.parse(item.gstAmount),
              cgstAmount: double.parse(item.cgstAmount),
              sgstAmount: double.parse(item.sgstAmount),
              igstAmount: double.parse(item.igstAmount),
              discountPercent: double.parse(item.discountPercent),
              discountAmount: double.parse(item.discountAmount),
              taxableAmount: double.parse(item.taxableAmount),
            ),
          )
          .toList(),
    );
