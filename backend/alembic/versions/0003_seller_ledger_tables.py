...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("opening_balance_customer_id", postgresql.UUID(as_uuid=True), nullable=True),
        ...
        sa.CheckConstraint("amount > 0", name="ck_customer_transactions_amount_positive"),
        sa.CheckConstraint(
        ...
        sa.UniqueConstraint("request_id", name="uq_customer_transactions_request_id"),
    )
    op.create_index("uq_customer_transactions_opening_balance", "customer_transactions", ["customer_id"], unique=True, postgresql_where=sa.text("entry_type = 'OPENING_BALANCE'"))


def downgrade() -> None:
    op.drop_index("uq_customer_transactions_opening_balance", table_name="customer_transactions")
