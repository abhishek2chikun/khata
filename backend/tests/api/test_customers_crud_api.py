...
def test_create_customer_rejects_duplicate_name_phone(client, auth_headers):
    payload = {
        "name": "ABC Stores",
        "address": "Market Yard",
        "phone": "9999999999",
    ...
    first = client.post("/customers", headers=auth_headers, json=payload)
    assert first.status_code == 201

    second = client.post("/customers", headers=auth_headers, json=payload)
