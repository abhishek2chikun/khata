...
SOURCE_CSV = REPO_ROOT / "data" / "source" / "MASTER CATALOG.csv"
SOURCE_XLSX = REPO_ROOT / "data" / "source" / "MASTER CATALOG.xlsx"


def test_source_workbook_tracked() -> None:
    assert SOURCE_CSV.exists() or SOURCE_XLSX.exists(), (
        "MASTER CATALOG.csv or MASTER CATALOG.xlsx must exist at data/source/"
    )
...
def test_generator_produces_matching_outputs() -> None:
    ...
    assert drift["catalog_version"] >= 7
    assert len(drift["products"]) == 1528
    assert len(drift["buyers"]) == 30
